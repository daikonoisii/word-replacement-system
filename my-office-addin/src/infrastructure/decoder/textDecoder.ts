...
export class CsvTextDecoderService implements ITextDecoderService {
  decode(file: File): Promise<string> {
    return file.arrayBuffer().then((arrayBuffer) => {
      ...
      if (
        ...
        // UTF-16 BE BOM
        decodedText = new TextDecoder('utf-16be').decode(
          uint8Array.subarray(2)
        );
      } else if (uint8Array[0] === 0xff && uint8Array[1] === 0xfe) {
        // UTF-16 LE BOM
        decodedText = new TextDecoder('utf-16le').decode(
          uint8Array.subarray(2)
        );
      } else {
        ...
        if (detected === 'UTF8') {
          decodedText = Encoding.codeToString(
